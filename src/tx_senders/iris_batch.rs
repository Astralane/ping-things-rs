...
impl TxSender for IrisBatchTxSender {
    ...
    async fn send_batch(
        ...
        let encoded_txns: Vec<String> = indices
            ...
            .map(|&(index, blockhash)| {
                let tx =
                    build_transaction_with_config(&self.tx_config, &RpcType::IrisBatch, index, blockhash);
        ...
        let result_array = parsed["result"]
            .as_array()
            .ok_or_else(|| anyhow::anyhow!("sendBatch response missing result array: {}", body_text))?;
        ...
        let signatures: Vec<TxResult> = result_array
            ...
            .map(|v| {
                ...
                let sig = Signature::from_str(sig_str)
                    .unwrap_or_else(|_| panic!("invalid signature in sendBatch response: {}", sig_str));
